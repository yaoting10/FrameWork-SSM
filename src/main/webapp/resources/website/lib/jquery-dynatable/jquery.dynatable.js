  ...
  processAll = function(skipPushState) {
    ...
    if (this.settings.dataset.ajax) {
      ...
      var options = {
        ...
        //TODO LXL FIT THE SRRING PAGEABLE PARAMS
        data: RebuildAjaxData(data),
    ...
    this.create = function() {
      ...
      if (settings.features.paginate) {
        ...
        if (options.recordsShown < options.recordsQueryCount) {
          ...
          options.pageLowerBound = (settings.dataset.page - 1)*settings.dataset.perPage + 1;//TODO LXL formate the display records
          options.pageUpperBound = (settings.dataset.page - 1)*settings.dataset.perPage + bounds[1];//TODO LXL formate the display records
        ...
        // Else if currently displayed records are the entire collection
            ...
            options.pageLowerBound = (settings.dataset.page - 1)*settings.dataset.perPage + 1;//TODO LXL formate the display records
            options.pageUpperBound = (settings.dataset.page - 1)*settings.dataset.perPage + options.recordsShown;//TODO LXL formate the display records
  ...
  function PaginationLinks(obj, settings) {
    ...
    this.create = function() {
      var pageLinks = '<ul id="' + 'dynatable-pagination-links-' + obj.element.id + '" class="' + settings.inputs.paginationClass + '">',
          ...
          //TODO LXL fix pagination problem
//          pages = Math.ceil(settings.dataset.queryRecordCount / settings.dataset.perPage),
  ...
  //TODO LXL FIT THE SRRING PAGEABLE PARAMS
  function RebuildAjaxData(data){
        var convert = {};
        convert.page = data.page - 1;
        convert.size = data.size;
        convert.offset = data.offset
        if(data.sorts != undefined){
            $.each(data.sorts, function(i, n)
            {
                convert["sort"] = i.toString() + "," + (n == 0 ? "desc":"asc");
            });
         }
         if(data.queries != undefined){
              $.each(data.queries, function(i, n)
              {
                  convert[i.toString()] = n;
              });
          }
        return convert;
  }
